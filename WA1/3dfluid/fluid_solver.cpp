  ...
  for (int l = 0; l < LINEARSOLVERTIMES; l++) { 
    for (int k = 1; k <= O; k++) {
      for (int j = 1; j <= N; j++) {
        int i;
        for (i = 1; i <= M-3; i+=4) {
          x[IX(i, j, k)] = (x0[IX(i, j, k)])*e +
                            d * (x[IX(i, j, k - 1)] + x[IX(i, j, k + 1)] +
                                 x[IX(i, j - 1, k)] + x[IX(i, j + 1, k)] +
                                 x[IX(i - 1, j, k)] + x[IX(i + 1, j, k)]);
          x[IX(i+1, j, k)] = (x0[IX(i+1, j, k)])*e +
                            d * (x[IX(i + 1, j, k - 1)] + x[IX(i + 1, j, k + 1)] +
                                 x[IX(i + 1, j - 1, k)] + x[IX(i + 1, j + 1, k)] +
                                 x[IX(i, j, k)] + x[IX(i + 2, j, k)]);
          x[IX(i+2, j, k)] = (x0[IX(i+2, j, k)])*e +
                            d * (x[IX(i + 2, j, k - 1)] + x[IX(i + 2, j, k + 1)] +
                                 x[IX(i + 2, j - 1, k)] + x[IX(i + 2, j + 1, k)] +
                                 x[IX(i + 1, j, k)] + x[IX(i + 3, j, k)]);
          x[IX(i+3, j, k)] = (x0[IX(i+3, j, k)])*e +
                            d * (x[IX(i + 3, j, k - 1)] + x[IX(i + 3, j, k + 1)] +
                                 x[IX(i + 3, j - 1, k)] + x[IX(i + 3, j + 1, k)] +
                                 x[IX(i + 2, j, k)] + x[IX(i + 4, j, k)]);
        }
       for (; i <= M; i++) {
          x[IX(i, j, k)] = (x0[IX(i, j, k)])*e +
                            d * (x[IX(i, j, k - 1)] + x[IX(i, j, k + 1)] +
